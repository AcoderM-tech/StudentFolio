    ...
    if request.method == 'POST':
        ...
        if password != tasdiqlash:
            return render(request, 'register.html', {
                'error': "Parol va parol tasdig'i teng emas!!",
                'username': username,
                'email': email
            })
...
def add_projects(request):
    ...
    if request.method == 'POST':
        ...
        avatar = request.FILES.get('avatar')  # .get() — fayl bo'lmasa xato bermaydi
...
def add_certificate(request):
    ...
    if request.method == 'POST':
        ...
        if nomi and olingan_sana and fayl_isbot:
            ...
            sertifikatlar = Sertifikat.objects.filter(profil=user_profil)
            return render(request, 'dashboard/certificates.html', {
                'certificates': sertifikatlar,
                'error': "Nomi, sana va fayl majburiy!",
                'profil': user_profil,
            })
    return redirect('certificates')
...
def add_achievement(request):
    ...
    if request.method == 'POST':
        ...
        for rasm in rasmlar:
            TadbirRasmi.objects.create(tadbir=tadbir, rasm=rasm)
...
def add_ish(request):
    ...
    if request.method == 'POST':
        ...
        fotolavha = request.FILES.get('fotolavha')
        if fotolavha:
            ish.fotolavha = fotolavha
            ish.save()
...
def del_job(request, pk):
    job = get_object_or_404(Ish_faoliyati, pk=pk, profil__user=request.user)
    job.delete()
    return redirect('experiences')
...
def edit_job(request, pk):
    ...
    if request.method == 'POST':
        ...
        job.tugatilgan_sana = tugash_sanasi if tugash_sanasi and tugash_sanasi.strip() else None
...
def edit_achievement(request, pk):
    ...
    if request.method == 'POST':
        ...
        achievement.bolib_otgan_sanasi = sana if sana and sana.strip() else None
        achievement.save()
